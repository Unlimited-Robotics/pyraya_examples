...
class RayaApplication(RayaApplicationBase):
    ...
    async def check_predefined_pose(self):
        if not self.predefined_pose in await \
                self.arms.get_list_predefined_poses(self.arm_name):
            await self.list_predefined_poses()
            raise ValueError(
                f'the predefined_pose {self.predefined_pose} is not available '
                'for this arm'
            )
    ...
    async def execute_predefined_pose(self, predefined_pose: str):
        self.log.info('')
        self.log.info(
            f'Start the execution of the predefined pose {predefined_pose}\n')

        await self.arms.set_predefined_pose(
            arm=self.arm_name,
            predefined_pose=predefined_pose,
